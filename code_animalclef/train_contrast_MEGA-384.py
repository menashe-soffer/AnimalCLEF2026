...
class SupConLoss(nn.Module):
    """Handles both SimCLR (unlabeled) and SupCon (labeled) based on labels."""
...
def train_contrastive(model, dataset, output_fname, epochs=5, lr=1e-4, batch_size=8, loss_type='SupConLoss', device='cuda'):
...
weak_train_transforms = T.Compose([
    UnderwaterEnhance,
    T.RandomResizedCrop(384, scale=(0.6, 1.0)),
...
TRN_PARAMS = dict()
TRN_PARAMS['SalamanderID2025'] = dict({'epochs': 5, 'transform': None, 'loss_type': 'simCLR', 'temperature': 0.07, 'val_ratio': 0.2})
TRN_PARAMS['SeaTurtleID2022'] = dict({'epochs': 5, 'transform': weak_train_transforms, 'loss_type': 'SupConLoss', 'temperature': 0.07, 'val_ratio': 0.2})
TRN_PARAMS['LynxID2025'] = dict({'epochs': 5, 'transform': weak_train_transforms, 'loss_type': 'SupConLoss', 'temperature': 0.07, 'val_ratio': 0.2})
TRN_PARAMS['TexasHornedLizards'] = dict({'epochs': 5, 'transform': None, 'loss_type': 'simCLR', 'temperature': 0.15, 'val_ratio': 0.2})



def main(subset=1):

    db_name = SUBSETS[subset]
    trn_params = TRN_PARAMS[db_name]
    print('\n\ntraining with dataset', db_name)

    loss_type = trn_params['loss_type']   # 'SupConLoss', 'simCLR'

    # 1. Setup Base Data
    dataset_full = AnimalCLEF2026(root=ROOT_DATA)
    # This subset can now include your 'test' split for lizards!
    base_subset = dataset_full.get_subset(dataset_full.df['dataset'] == SUBSETS[subset])
    ...
    output_path = os.path.join(ROOT_MODELS,'mega384_refined_{}_{}.pth'.format(loss_type, SUBSETS[subset]))
